...
class Templator:
    ...
    def genVXLAN(self,servers,targets,v6=False):
        ...
        for node,data in servers.items():
            if v6:
                template += f'bridge fdb append 00:00:00:00:00:00 dev vxlan{targets["vxlanID"]}v6 dst fc10:0:0:{data["id"]}::1;'
            else:
        ...
        for client in clients:
            if v6:
                template += f'bridge fdb append 00:00:00:00:00:00 dev vxlan{targets["vxlanID"]}v6 dst fc10:0:250:{count}::1;'
            else:
    ...
    def genServer(self,targets,ip,data,server,port,privateKey,publicKey,resolve):
        ...
        if port == data['basePort']:
            template += f'\nPostUp =  echo 1 > /proc/sys/net/ipv4/ip_forward; echo 1 > /proc/sys/net/ipv6/conf/all/forwarding; echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter; echo 0 > /proc/sys/net/ipv4/conf/default/rp_filter; ip addr add {targets["prefixSub"]}.{data["id"]}.1/30 dev lo; ip addr add fc10:0:0:{data["id"]}::1/48 dev lo;'
            ...
            #vxlan v6
            randomMac = "52:54:00:%02x:%02x:%02x" % (random.randint(0, 255),random.randint(0, 255),random.randint(0, 255),)
            template += f'ip -6 link add vxlan{targets["vxlanID"]}v6 type vxlan id {targets["vxlanID"]+1} dstport {targets["vxlanID"]+1}789 local fc10:0:0:{data["id"]}::1; ip -6 link set vxlan{targets["vxlanID"]}v6 up;'
            template += f'ip -6 link set dev vxlan{targets["vxlanID"]}v6 address {randomMac};'
            template += f'ip -6 addr add fc10:0:{targets["vxlanSub"]}::{data["id"]}/48 dev vxlan{targets["vxlanID"]}v6;'
            template += self.genVXLAN(targets['servers'],targets,True)
            #postDown
            template += f'\nPostDown = ip addr del {targets["prefixSub"]}.{data["id"]}.1/30 dev lo; ip addr del fc10:0:0:{data["id"]}::1/48 dev lo; ip link delete vxlan{targets["vxlanID"]}; ip link delete vxlan{targets["vxlanID"]}v6;'
        template += f'''
        SaveConfig = false
        Table = off
        [Peer]
    ...
    def genClient(self,targets,ip,subnet,server,port,privateKey,publicKey,clientIP,clients,client):
        ...
        if clientIP == True:
            ...
            template += f'\nPostUp =  echo 1 > /proc/sys/net/ipv4/ip_forward; echo 1 > /proc/sys/net/ipv6/conf/all/forwarding; echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter; echo 0 > /proc/sys/net/ipv4/conf/default/rp_filter; echo "fq" > /proc/sys/net/core/default_qdisc; echo "bbr" > /proc/sys/net/ipv4/tcp_congestion_control; ip addr add {targets["prefixSub"]}.250.{len(clients)}/32 dev lo; ip addr add fc10:0:250:{len(clients)}::1/48 dev lo;'
            #vxlan v4
            ...
            template += self.genVXLAN(targets['servers'],targets)
            #vxlan v6
            template += f'ip -6 link add vxlan{targets["vxlanID"]}v6 type vxlan id {targets["vxlanID"]+1} dstport {targets["vxlanID"]+1}789 local fc10:0:250:{len(clients)}::1; ip -6 link set vxlan{targets["vxlanID"]}v6 up;'
            template += f'ip -6 addr add fc10:0:{targets["vxlanSub"]}::{vxlanIP}/48 dev vxlan{targets["vxlanID"]}v6;'
            template += self.genVXLAN(targets['servers'],targets,True)
            #postDown
            template += f'\nPostDown = ip addr del {targets["prefixSub"]}.250.{len(clients)}/32 dev lo; ip addr del fc10:0:250:{len(clients)}::1/48 dev lo; ip link delete vxlan{targets["vxlanID"]}; ip link delete vxlan{targets["vxlanID"]}v6;'
        template += f'''
        SaveConfig = false
        Table = off
        [Peer]
