...
class Templator:
    ...
    def genServer(self,targets,ip,data,server,port,privateKey,publicKey,resolve):
        ...
        if port == data['basePort']:
            template += f'\nPostUp =  echo 1 > /proc/sys/net/ipv4/ip_forward; echo 1 > /proc/sys/net/ipv6/conf/all/forwarding; echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter; echo 0 > /proc/sys/net/ipv4/conf/default/rp_filter; ip addr add {targets["prefixSub"]}.{data["id"]}.1/30 dev lo; ip addr add fc10:0:0:{data["id"]}::1/64 dev lo;'
            ...
            template += f'ip -6 addr add fc10:0:{targets["vxlanSub"]}::{data["id"]}/64 dev vxlan{targets["vxlanID"]}v6;'
            template += self.genVXLAN(targets['servers'],targets,True)
            #postDown
            template += f'\nPostDown = ip addr del {targets["prefixSub"]}.{data["id"]}.1/30 dev lo; ip addr del fc10:0:0:{data["id"]}::1/64 dev lo; ip link delete vxlan{targets["vxlanID"]}; ip link delete vxlan{targets["vxlanID"]}v6;'
        template += f'''
        SaveConfig = false
        Table = off
        [Peer]
    ...
    def genClient(self,targets,ip,subnet,server,port,privateKey,publicKey,clientIP,clients,client):
        ...
        if clientIP == True:
            ...
            template += f'\nPostUp =  echo 1 > /proc/sys/net/ipv4/ip_forward; echo 1 > /proc/sys/net/ipv6/conf/all/forwarding; echo 0 > /proc/sys/net/ipv4/conf/all/rp_filter; echo 0 > /proc/sys/net/ipv4/conf/default/rp_filter; echo "fq" > /proc/sys/net/core/default_qdisc; echo "bbr" > /proc/sys/net/ipv4/tcp_congestion_control; ip addr add {targets["prefixSub"]}.250.{len(clients)}/32 dev lo; ip addr add fc10:0:250:{len(clients)}::1/64 dev lo;'
            #vxlan v4
            ...
            template += f'ip -6 addr add fc10:0:{targets["vxlanSub"]}::{vxlanIP}/64 dev vxlan{targets["vxlanID"]}v6;'
            template += self.genVXLAN(targets['servers'],targets,True)
            #postDown
            template += f'\nPostDown = ip addr del {targets["prefixSub"]}.250.{len(clients)}/32 dev lo; ip addr del fc10:0:250:{len(clients)}::1/64 dev lo; ip link delete vxlan{targets["vxlanID"]}; ip link delete vxlan{targets["vxlanID"]}v6;'
        template += f'''
        SaveConfig = false
        Table = off
        [Peer]
